...
var output = _config.PROJECT_PROD_OUTPUT || 'dist';

if (process.env.RUN_TEST) {
  v = _config.VERSION + '.release';
  output = _config.PROJECT_RELEASE_OUTPUT;
}

if (process.env.RUN_DEV) {
  v = _config.VERSION + '.dev';
  output = _config.PROJECT_DEV_OUTPUT;
}
...
_webpack5.default.output = {
  ...
  path: _path2.default.resolve(_config.PROJECT_PATH, output),
  publicPath: '/'
};
...
_webpack5.default.plugins.push(new _webpack3.default.DefinePlugin({
  ...
  'process.env.MODULE': (0, _stringify2.default)(_config.MODULES)
}));
...
_webpack5.default.plugins.push(new _webpack3.default.optimize.CommonsChunkPlugin({
  names: _lodash2.default.keys(_config.VENDOR).concat(['manifest']),
  minChunks: Infinity
}));
