...
*/
var urls = _lodash["default"].map(_lodash["default"].filter(_flyDll.dll.manifest, {
  type: 'js'
}), 'url');

var mfs = _lodash["default"].map(_lodash["default"].filter(_flyDll.dll.manifest, {
  type: 'json'
}), 'filename');
...
for (var i = 0; i < mfs.length; i += 1) {
  var p = "fly-dll/dll/".concat(mfs[i]);
  drps.push(require(p));
}
/**
 * [config description]
